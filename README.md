...
Auto-activate every session: Claude Code, Gemini (built-in), opencode, OpenClaw. Codex is per-session: use `/caveman` each time. Cursor / Windsurf / Cline / Copilot get always-on rule files via `--with-init`, so they start in your configured default mode (ultra when nothing overrides it) while caveman is installed. Other agents trigger with `/caveman` per session. Full feature matrix in [INSTALL.md](./INSTALL.md#what-you-get).
...
3. For Claude Code, hook also write tiny flag file each session — agent see flag, start in your configured default mode (ultra when nothing overrides it) while caveman is installed, until you switch modes or say `normal mode`. No need say `/caveman`.
